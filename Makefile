...
lint::
	PYTHONDONTWRITEBYTECODE=1 '$(REPOSITORY_PYTHON_LITERAL)' -m py_compile '$(REPOSITORY_ROOT_LITERAL)/scripts/check_whattowear_contracts.py' '$(REPOSITORY_ROOT_LITERAL)/scripts/test_whattowear_mutations.py'

test::
	PYTHONDONTWRITEBYTECODE=1 '$(REPOSITORY_PYTHON_LITERAL)' '$(REPOSITORY_ROOT_LITERAL)/scripts/check_whattowear_contracts.py'
	PYTHONDONTWRITEBYTECODE=1 '$(REPOSITORY_PYTHON_LITERAL)' '$(REPOSITORY_ROOT_LITERAL)/scripts/test_whattowear_mutations.py'

build::
	@if [ -x '$(REPOSITORY_XCODEBUILD_LITERAL)' ]; then \
		'$(REPOSITORY_XCODEBUILD_LITERAL)' -project '$(REPOSITORY_ROOT_LITERAL)/What To Wear.xcodeproj' -scheme 'What To Wear' -configuration Debug CODE_SIGNING_ALLOWED=NO build; \
	else \
		/usr/bin/printf '%s\n' 'Skipping xcodebuild: xcodebuild is not installed.'; \
	fi
